...
def SelfRoot(n):
    dn = n + 20
    return np.random.normal(loc=0, scale=(pow(dn,(4/dn)) - 1))
            ...
            if type(LayLis[i+1]) == str:
                ...
                for j in range(LayLis[i]):
                    ...
                    if type(LayLis[i+1]) == str:
                        ...
                        for k in range(MakeThis):
                            ...
                            srtTofile[j].append(SelfRoot(MakeThis))#np.random.normal(loc=0, scale=math.sqrt(2/int(pow(math.sqrt(LayLis[i + addtopool])-(addtopool-1), 2)))))
                ...
                for j in range(LayLis[i]):
                    ...
                    for k in range(LayLis[i + 1]):
                        ...
                        srtTofile[j].append(SelfRoot(LayLis[i + 1]))#np.random.normal(loc=0, scale=math.sqrt(2/LayLis[i + 1])))
