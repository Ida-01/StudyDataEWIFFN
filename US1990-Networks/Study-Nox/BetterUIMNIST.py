...
def TestingNetwork(InputData, ExpeOutput, TrainingVal, BatchValTrue, Textra):
    ...
    AvrageERR = open('ErrorAvrage' + Textra + '.txt', 'a') 
    AvrageERR.write(str(sum(MSElist)/len(MSElist)) + "\n")
    AvrageERR.close()

    LowestERR = open('ErrorLowest' + Textra + '.txt', 'a') 
    LowestERR.write(str(np.min(MSElist)) + "\n")
    LowestERR.close()
