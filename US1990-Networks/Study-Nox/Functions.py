...
def NormalXavier(n, m):
    Fval = math.sqrt(6)/(math.sqrt(n+m))
    return np.random.uniform(low=-Fval, high=Fval)
            ...
            if type(LayLis[i+1]) == str:
                ...
                for j in range(LayLis[i]):
                    ...
                    if type(LayLis[i+1]) == str:
                        ...
                        for k in range(MakeThis):
                            ...
                            srtTofile[j].append(NormalXavier(MakeThis, LayLis[i]))#np.random.normal(loc=0, scale=math.sqrt(2/int(pow(math.sqrt(LayLis[i + addtopool])-(addtopool-1), 2)))))
                ...
                for j in range(LayLis[i]):
                    ...
                    for k in range(LayLis[i + 1]):
                        ...
                        srtTofile[j].append(NormalXavier(LayLis[i + 1], LayLis[i]))#np.random.normal(loc=0, scale=math.sqrt(2/LayLis[i + 1])))
